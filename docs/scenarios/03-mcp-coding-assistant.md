...
|------|-------|---------|---------|-------|
| IMP-01 | access_count 自增 | 应用层维护 | retrieve 成功后自动自增 | 中 |
| IMP-02 | daily 文件清理 | 永不删除（可追溯） | 提供 `cleanup_daily` 配置项 | 低 |
| IMP-03 | 评分维度扩展 | 3 维（topic_relevance=0） | 接入 LLM 实现主题相关性 | 中 |
| IMP-04 | 寒暄词典扩展 | 内置 ~20 个模式 | 支持用户自定义词典 | 低 |
| IMP-05 | 月级 clue_anchors 生成 | 从 tags 提取 | LLM 生成更精准锚点 | 中 |
