...
**文档版本**：2026-07-16 v3（含 v2.54 P24 系数校准：Claude 1.05→1.20、DeepSeek 1.1→0.95；v2.54 P28 新增 OpenCode Zen 计划 5 个型号 + 4 个新家族）
...
**归档阈值优先级链**（高 → 低，v2.54 P20 修正：与 `builder.rs` 实际实现对齐）：

```
用户显式参数 > Scenario > Model > 默认 400K
```

> **历史说明**：早期文档承诺 7 层优先级（用户 > Scenario > Model > Window > Skill > Agent > 默认），
> 但 `archive_threshold` 实际只实现 4 层。Window/Skill/Agent 三层不参与阈值解析，仅参与其他字段：
> - **Window**：`archive_to_memory_center` 联合判断 + Agent→Window 联动推导
> - **Agent**：`session_prefix` / `archive_to_memory_center` / `usage_protocol` 生成
> - **Skill**：当前未参与阈值解析（预留扩展）
>
> v2.54 P18 引入 Scenario/Model 协商：解析阈值 > `model.context_window × 0.8` 时降级（用户显式阈值不受影响）。
...
| Doubao | 字节跳动豆包 | CharacterBased |
| MiniMax | MiniMax | CharacterBased |
| Kimi | 月之暗面 Kimi | CharacterBased |
| Glm | 智谱 GLM | ClaudeApprox |
| Hunyuan | 腾讯混元 Hunyuan | CharacterBased |
| Mimo | 小米 MiMo | CharacterBased |
| Nimotron | NVIDIA Nimotron | CharacterBased |
| North | Cohere North | CharacterBased |
| Local | 本地模型 | CharacterBased |
| Custom | 自定义模型 | CharacterBased |

> 备注：
> - Gemini/Qwen/Llama 默认用 `TokenizerKind::spm_or_char()` 智能选择 —— 启用 `tokenizer-sentencepiece` feature 时用 SentencePiece 真实分词器，未启用时降级 CharacterBased 兜底（v2.53 P9 实现，详见 [sentencepiece-guide.md](sentencepiece-guide.md)）。
> - v2.54 P26 新增 Doubao/MiniMax/Kimi/Glm 四个家族，对应 Trae 内置 12 个型号（统一限制 200K 上下文）。
> - Doubao/MiniMax/Kimi 的 tokenizer 未开源，暂用 CharacterBased 兜底（待 P24 实测替换）。
> - Glm 与 Claude 分词接近，暂用 ClaudeApprox 近似（v2.54 P24 已将 ClaudeApprox 系数从 1.05 调整为 1.20，Glm 同步跟随）。
> - **v2.54 P28 新增**：Hunyuan/Mimo/Nimotron/North 四个家族，对应 OpenCode Zen 计划 5 个型号（各有上下文限制 190K/200K/256K/1M，与 Trae 统一 200K 模式不同）。四家族的 tokenizer 均未开源，暂用 CharacterBased 兜底；DeepSeek 复用 `DeepSeekApprox`（OpenCode 版 DeepSeek-V4-Flash）。

### 表 5.2 ModelVariant 内置型号（32 个 = 15 原生 + 12 Trae + 5 OpenCode）

32 个构造器逐个提取，数据 100% 来自源码（v2.54 P26/P27/P28 更新）。
...
#### 5.2.2 Trae 内置型号（12 个，v2.54 P26 新增 / P27 能力标记更新）

| 构造器名 | name | family | context_window | tokenizer | supports_thinking | supports_vision | supports_audio | tool_call_format | archive_strategy | summary_max_tokens |
|---|---|---|---|---|---|---|---|---|---|---|
| trae_doubao_seed_2_1_pro | doubao-seed-2.1-pro | Doubao | 200,000 | CharacterBased | true | true | false | OpenAI | LargeWindow(50,000) | 1024 |
| trae_doubao_seed_2_1_turbo | doubao-seed-2.1-turbo | Doubao | 200,000 | CharacterBased | true | true | false | OpenAI | LargeWindow(50,000) | 1024 |
| trae_doubao_seed_code | doubao-seed-code | Doubao | 200,000 | CharacterBased | false | false | false | OpenAI | LargeWindow(50,000) | 1024 |
| trae_minimax_m3 | minimax-m3 | MiniMax | 200,000 | CharacterBased | true | true | false | OpenAI | LargeWindow(50,000) | 1024 |
| trae_glm_5_2 | glm-5.2 | Glm | 200,000 | ClaudeApprox | true | true | false | OpenAI | LargeWindow(50,000) | 1024 |
| trae_glm_5_1 | glm-5.1 | Glm | 200,000 | ClaudeApprox | true | true | false | OpenAI | LargeWindow(50,000) | 1024 |
| trae_glm_5 | glm-5 | Glm | 200,000 | ClaudeApprox | true | false | false | OpenAI | LargeWindow(50,000) | 1024 |
| trae_deepseek_v4_pro | trae-deepseek-v4-pro | DeepSeek | 200,000 | DeepSeekApprox | true | false | false | OpenAI | LargeWindow(50,000) | 1024 |
| trae_deepseek_v4_flash | trae-deepseek-v4-flash | DeepSeek | 200,000 | DeepSeekApprox | true | false | false | OpenAI | LargeWindow(50,000) | 1024 |
| trae_kimi_k2_7_code | kimi-k2.7-code | Kimi | 200,000 | CharacterBased | false | false | false | OpenAI | LargeWindow(50,000) | 1024 |
| trae_kimi_k2_6 | kimi-k2.6 | Kimi | 200,000 | CharacterBased | false | false | false | OpenAI | LargeWindow(50,000) | 1024 |
| trae_qwen_3_7_plus | trae-qwen-3.7-plus | Qwen | 200,000 | CharacterBased | true | true | false | OpenAI | LargeWindow(50,000) | 1024 |

#### 5.2.3 OpenCode Zen 计划型号（5 个，v2.54 P28 新增）

OpenCode Zen 计划为免费层，各型号有独立上下文限制（与 Trae 统一 200K 限制模式不同）。付费 Go 计划上下文应该不会做限制，未来接入时另行构造器。

| 构造器名 | name | family | context_window | tokenizer | supports_thinking | supports_vision | supports_audio | tool_call_format | archive_strategy | summary_max_tokens |
|---|---|---|---|---|---|---|---|---|---|---|
| opencode_deepseek_v4_flash | opencode-deepseek-v4-flash | DeepSeek | 200,000 | DeepSeekApprox | true | false | false | OpenAI | LargeWindow(50,000) | 1024 |
| opencode_hy3 | opencode-hy3 | Hunyuan | 190,000 | CharacterBased | true | true | false | OpenAI | Standard(47,500) | 1024 |
| opencode_mimo_v2_5 | opencode-mimo-v2.5 | Mimo | 200,000 | CharacterBased | true | true | false | OpenAI | LargeWindow(50,000) | 1024 |
| opencode_nimotron_3_ultra | opencode-nimotron-3-ultra | Nimotron | 1,000,000 | CharacterBased | true | false | false | OpenAI | LargeWindow(250,000) | 1024 |
| opencode_north_mini_code | opencode-north-mini-code | North | 256,000 | CharacterBased | false | false | false | OpenAI | LargeWindow(64,000) | 1024 |
...
>
> **v2.54 P26/P27 说明**：
> - 12 个 Trae 内置型号统一限制 200K 上下文，`archive_strategy` 统一 `LargeWindow(50,000)`（v2.54 P22 推导规则：200K / 4 = 50K）。
> - `summary_max_tokens` 统一 1024（200K 上下文属中模型，按差异化策略取 1024）。
> - **P27 能力标记更新**（基于 2026-07 官方文档调研）：
>   - `doubao-seed-2.1-pro/turbo`：thinking+vision 双双 true（旗舰深度推理版 + 多模态图文视频深度理解，原生 256K）
>   - `minimax-m3`：thinking+vision 双双 true（推理能力 + 原生多模态，原生 1M）
>   - `glm-5.2/5.1`：vision true（5.x 旗舰版延续多模态）
>   - `glm-5`：vision 保持 false（保守，初代未明确多模态）
>   - `trae-deepseek-v4-flash`：thinking true（V3.1 起混合模型路线）
>   - `trae-qwen-3.7-plus`：thinking+vision 双双 true（全域思考模式 + 原生多模态，原生 1M）
>   - `doubao-seed-code` / `kimi-k2.7-code` / `kimi-k2.6`：保持保守（代码/对话模型未明确宣传思考链/多模态）
> - **tokenizer 选择依据**：
>   - GLM 系列用 `ClaudeApprox`（与 Claude 分词接近，系数 1.20，v2.54 P24）
>   - 其他 9 个型号用 `CharacterBased` 兜底（厂商 tokenizer 未开源，P27 范围不引入新依赖，待 P24 实测后替换）
>
> **v2.54 P28 说明**（OpenCode Zen 计划）：
> - **5 个 OpenCode Zen 型号各有独立上下文限制**（190K/200K/200K/1M/256K），与 Trae 统一 200K 模式不同。
> - `archive_strategy` 按 v2.54 P22 推导规则（`window / 4`）：
>   - 190K → `Standard(47,500)`（<200K 走 Standard 分支）
>   - 200K → `LargeWindow(50,000)`（DeepSeek/Mimo 各一个）
>   - 256K → `LargeWindow(64,000)`
>   - 1M → `LargeWindow(250,000)`
> - `summary_max_tokens` 统一 1024（与 Trae 内置型号一致策略）。
> - **能力标记依据**（基于 2026-07 官方文档调研）：
>   - `opencode-deepseek-v4-flash`：thinking true（DeepSeek V3.1+ 路线）、vision false
>   - `opencode-hy3`（腾讯混元）：thinking+vision 双双 true（295B MoE 推理优化 + 多模态）
>   - `opencode-mimo-v2.5`（小米 MiMo）：thinking+vision 双双 true（全模态 Agent 模型）
>   - `opencode-nimotron-3-ultra`（NVIDIA）：thinking true（MoE Hybrid Mamba-Transformer，Agentic Reasoning）、vision false
>   - `opencode-north-mini-code`（Cohere）：thinking+vision 双双 false（30B MoE / 3B 活跃参数的纯代码模型，Apache 2.0 开源）
> - **tokenizer 选择依据**：
>   - `opencode-deepseek-v4-flash` 复用 `DeepSeekApprox`（与 DeepSeek 家族共享 tokenizer，v2.54 P24 系数 0.95）
>   - 其余 4 个新家族（Hunyuan/Mimo/Nimotron/North）的 tokenizer 均未开源，暂用 `CharacterBased` 兜底
> - **三版本 DeepSeek-V4-Flash 互异**：
>   - `deepseek-v4-flash`（原生版）：1M 上下文
>   - `trae-deepseek-v4-flash`（Trae 限制版）：200K 上下文
>   - `opencode-deepseek-v4-flash`（OpenCode Zen 版）：200K 上下文（与 Trae 版同窗口但不同构造器，便于未来差异化调整）
...
| ClaudeApprox | "claude_approx" | Claude 近似（cl100k_base + 系数 1.20，v2.54 P24 推理暂定） | 1.20 |
| DeepSeekApprox | "deepseek_approx" | DeepSeek 近似（cl100k_base + 系数 0.95，v2.54 P24 实测校准） | 0.95 |
...
> 备注：v2.54 P24 系数校准详情：
> - **ClaudeApprox 1.05 → 1.20**：基于公开资料推理（Sonnet 5 / Opus 4.7+ 共享新 tokenizer，官方系统卡承认涨幅 1.0~1.35 倍，中位约 1.17，叠加旧系数 1.05 取保守暂定 1.20），待 OpenRouter 充值后实测校准。
> - **DeepSeekApprox 1.1 → 0.95**：DeepSeek 官方 API 实测 100 条中英代码混合样本，V4 Pro/Flash 共享同一 tokenizer，整体平均系数 1.0115（被短样本 API 开销拉高），长文本平均约 0.94（更贴近归档场景），取 0.95 作为折中值。DeepSeek V4 对中文优化明显（中文 token 比 cl100k 少 25-40%）。
> - **应用条件**：`count_tokens` 中系数判断从 `> 1.0` 改为 `!= 1.0`，支持 DeepSeek 系数 < 1.0 的情况。
> - **测试覆盖**：`tiktoken_impl.rs` 新增 `test_p24_claude_coefficient_value` / `test_p24_deepseek_coefficient_value` 两个精度验证测试，原 `test_deepseek_approx_higher_than_cl100k` 改名为 `test_deepseek_approx_lower_than_cl100k`（断言方向反转）。
> - **校准工具**：`examples/calibrate_tokens`（独立 workspace member，`publish = false`），支持 OpenRouter + DeepSeek 官方两个 provider，详见 [表 5.9](#表-59-p24-校准工具examplescalibrate_tokens)。
...
| Kimi | kimi-k2.7-code |
| Glm | glm-5.2 |
| Hunyuan | opencode-hy3 |
| Mimo | opencode-mimo-v2.5 |
| Nimotron | opencode-nimotron-3-ultra |
| North | opencode-north-mini-code |
| Local | local-default |
| Custom | custom（`custom("custom", Custom, 32_000)`） |

> 备注：
> - Claude 默认选 Opus 4.8 而非 Fable 5/Mythos 5，原因：Fable 5 曾因出口管制暂停，Mythos 5 面向特定合作方，Opus 4.8 为 API 普遍可用的稳定旗舰。
> - v2.54 P26 新增 4 个家族（Doubao/MiniMax/Kimi/Glm），当前仅有 Trae 内置版本，默认返回 Trae 限制版（200K）。未来若有原生版本接入，可调整默认值。
> - **v2.54 P28 新增 4 个家族**（Hunyuan/Mimo/Nimotron/North），当前仅有 OpenCode Zen 版本，默认返回 Zen 限制版（各有上下文限制）。未来若有 Go 计划付费版接入，可调整默认值。
> - v2.54 P25 新增 `-latest` 别名机制：`find("<family>-latest")` 自动转发到 `default_variant(family)`，共支持 15 个家族别名（`claude-latest` / `gpt-latest` / `gemini-latest` / `deepseek-latest` / `qwen-latest` / `llama-latest` / `grok-latest` / `doubao-latest` / `minimax-latest` / `kimi-latest` / `glm-latest` / `hunyuan-latest` / `mimo-latest` / `nimotron-latest` / `north-latest`）。别名与具体型号解耦，`default_variant()` 升级时别名自动跟随。无 `local-latest` / `custom-latest`（无家族别名需求）。

### 表 5.6.1 ModelVariant.deprecated 字段（v2.54 P25 新增）

`deprecated: Option<&'static str>` 字段标记型号废弃状态：

| 值 | 含义 | 影响 |
|---|---|---|
| `None` | 活跃型号，推荐使用 | 正常输出 |
| `Some(原因)` | 已废弃，建议迁移 | `preset_list_models` 输出标记，`find()` 仍可查找（向后兼容） |

> 序列化说明：
> - `Serialize`：正常输出（None → null，Some(s) → 字符串）
> - `Deserialize`：标注 `#[serde(skip_deserializing)]`，反序列化时默认为 None。原因：`&'static str` 无法从反序列化输入构造，该字段由服务端构造器权威设置。当前所有 32 个内置型号均为 `None`（活跃状态）。
...
| ✅ Claude/DeepSeek 系数校准（v2.54 P24） | 已实现：Claude 1.05→1.20（推理暂定）+ DeepSeek 1.1→0.95（实测校准）+ 应用条件从 `> 1.0` 改为 `!= 1.0` + 2 个精度验证测试 + 校准工具 `examples/calibrate_tokens`（详见 [表 5.9](#表-59-p24-校准工具examplescalibrate_tokens)） |
| ✅ OpenCode Zen 计划型号扩充（v2.54 P28） | 已实现：新增 4 个 ModelFamily（Hunyuan/Mimo/Nimotron/North）+ 5 个 OpenCode 构造器（各有独立上下文限制 190K/200K/256K/1M）+ 4 个 `-latest` 别名 + 4 个 default_variant 分支。家族总数 13→17，型号总数 27→32。DeepSeek-V4-Flash 出现三版本互异（原生 1M / Trae 200K / OpenCode 200K） |
...
| OpenRouter 充值后实测 Claude | 待 OpenRouter 充值后运行 `calibrate_tokens --model claude-opus-4.8` / `--model claude-sonnet-5`，校准暂定值 1.20 |
| OpenCode Go 计划付费版接入 | 当前仅有 Zen 计划（免费层），Go 计划（付费层）上下文应该不会做限制，未来接入时需新增 `opencode_go_*` 构造器并调整 default_variant |

### 表 5.9 P24 校准工具（examples/calibrate_tokens）

v2.54 P24 新增的独立 workspace member（`publish = false`，不发布到 crates.io），通过 LLM Provider API 实测真实 `prompt_tokens`，对比 tiktoken-rs `cl100k_base` 原始计数，反推各型号的实际系数。

源码：`examples/calibrate_tokens/`

#### 5.9.1 支持的 Provider

| Provider | API 端点 | 环境变量 | 适用模型 | 备注 |
|---|---|---|---|---|
| OpenRouter | `https://openrouter.ai/api/v1/chat/completions` | `OPENROUTER_API_KEY` | Claude 系列 | 需额外 `HTTP-Referer` 与 `X-Title` 请求头 |
| DeepSeek 官方 | `https://api.deepseek.com/v1/chat/completions` | `DEEPSEEK_API_KEY` | DeepSeek V4 系列 | OpenAI 兼容格式，无需额外请求头 |

#### 5.9.2 支持的模型映射

| 简称（`--model` 参数） | Provider | API 模型 ID |
|---|---|---|
| `claude-opus-4.8` | OpenRouter | `anthropic/claude-opus-4.8` |
| `claude-sonnet-5` | OpenRouter | `anthropic/claude-sonnet-5` |
| `deepseek-v4-pro` | DeepSeek 官方 | `deepseek-v4-pro` |
...
|---|---|---|
| `--model` | 必填 | 模型简称（见上表） |
| `--api-model-id` | 由 `--model` 推导 | 显式覆盖 API 模型 ID |
| `--samples` | `fixtures/calibration/samples.jsonl` | 100 条 JSONL 样本路径 |
| `--output-dir` | `fixtures/calibration/reports/` | 报告输出目录（已 gitignore） |
| `--api-key` | 从环境变量读取 | 显式覆盖 API Key（不写入文件） |
| `--delay-ms` | 500 | 请求间隔毫秒数（避免速率限制） |
| `--timeout-secs` | 60 | 单次请求超时秒数 |
| `--limit` | 无 | 限制样本数（用于快速验证） |
| `--dry-run` | false | 仅打印不调用 API |

#### 5.9.4 重试机制

| 错误类型 | 处理策略 |
|---|---|
| 429 速率限制 | 自动重试（最多 3 次，指数退避） |
| 5xx 服务端错误 | 自动重试（最多 3 次，指数退避） |
| 4xx 客户端错误 | 不重试，直接返回错误 |
| 网络超时 | 自动重试（最多 3 次） |

#### 5.9.5 报告输出

Markdown 格式输出到 `fixtures/calibration/reports/<model>-<timestamp>.md`（已 gitignore），包含 4 章节：

1. **总体结论**：平均系数、建议系数、最小/最大系数、标准差、平均偏差
2. **偏差分布**：±5% / ±10% / ±15% / ±20% / >20% 各区间样本占比
3. **按类别分组统计**：30+ 类别的平均系数与偏差
4. **详细样本数据**：每条样本的 cl100k_base 计数 / 实测 token / 系数 / 偏差 / 文本预览
...
# Dry run（不调用 API，仅验证样本读取）
cargo run -p calibrate_tokens -- --model deepseek-v4-pro --dry-run
```

#### 5.9.7 v2.54 P24 实测结论

| 模型 | 实测样本 | 整体平均系数 | 长文本系数 | 采用系数 | 备注 |
|---|---|---|---|---|---|
| deepseek-v4-pro | 100 条 | 1.0115 | ≈ 0.94 | **0.95** | 与 Flash 共享 tokenizer |
| deepseek-v4-flash | 100 条 | 1.0115 | ≈ 0.94 | **0.95** | 与 Pro 共享 tokenizer |
| claude-opus-4.8 | 未实测 | — | — | **1.20**（暂定） | 待 OpenRouter 充值后实测 |
| claude-sonnet-5 | 未实测 | — | — | **1.20**（暂定） | 待 OpenRouter 充值后实测 |

> 关键发现：
> - DeepSeek V4 Pro 和 V4 Flash 实测数据完全一致（通过报告比对确认），证明共享同一 tokenizer
> - DeepSeek V4 对中文优化显著（中文 token 比 cl100k 少 25-40%），与原假设"中文场景偏差 ±10-20%"方向相反
> - 短样本受 API 固定开销影响（约 4 token），整体平均系数被拉高，长文本场景更准确为 0.94
...
| models | Claude/DeepSeek 系数校准 | ✅ 已于 v2.54 P24 实现（Claude 1.05→1.20 推理暂定，DeepSeek 1.1→0.95 实测校准，详见 [表 5.5](#表-55-tokenizerkind-枚举) 与 [表 5.9](#表-59-p24-校准工具examplescalibrate_tokens)） | 高（影响归档精度） |
| models | OpenCode Zen 计划型号扩充 | ✅ 已于 v2.54 P28 实现（新增 4 家族 Hunyuan/Mimo/Nimotron/North + 5 个 OpenCode 构造器 + 4 个别名 + 4 个 default_variant 分支，家族 13→17 / 型号 27→32，详见 [表 5.1](#表-51-modelfamily-枚举17-家族) / [表 5.2.3](#523-opencode-zen-计划型号5-个v254-p28-新增) / [表 5.6](#表-56-modelregistry-默认型号映射)） | 中（已完成） |
...
> 本文档由源码同步，如配置变更请同步更新。最后更新：2026-07-16（v2.54 P24 系数校准）。
