...
**Tech Stack:** Rust + async-trait + tokio + serde + reqwest（复用 memory-center-llm 的 `LlmDetectorConfig`）；TDD（每任务先写失败测试再实现）。
...
| 文件 | 改动类型 | 责任 |
|------|---------|------|
| `crates/memory-center-core/src/storage.rs` | 修改 | 新增 `SessionMeta` struct + Storage trait 2 个新方法（默认实现）+ LocalStorage 实现 |
| `crates/memory-center-core/src/sqlite.rs` | 修改 | SqliteStorage 实现：新增 `session_meta` 表 + 2 个方法 |
| `crates/memory-center-core/src/cache.rs` | 修改 | CachedStorage 透传实现（避免默认实现使包装失效） |
| `crates/memory-center-presets/src/builder.rs` | 修改 | 新增 `scenario_to_str` + 扩展 `scenario_from_str` 支持 `custom:` 前缀 |
...
| `crates/memory-center-presets/src/lib.rs` | 修改 | 导出新模块 + 重导出 API |
| `crates/memory-center-presets/Cargo.toml` | 修改 | 新增 memory-center-llm 依赖（HttpScenarioDetector 用） |
| `crates/memory-center-mcp/src/lib.rs` | 修改 | MemoryCenterMcp 新增 `scenario_detector` 字段 + `with_scenario_detector` 链式方法 + archive handler 调用 `resolve_effective_scenario` |
| `crates/memory-center-mcp/src/main.rs` | 修改 | 新增 `build_scenario_detector` 函数 + 注入 MemoryCenterMcp |

**不修改的部分**：
- `crates/memory-center-scenarios/*` — 场景数据 crate 保持纯数据
- `crates/memory-center-core/src/archive.rs` — Archiver 本身不改
...
**Files:**
- Modify: `crates/memory-center-core/src/storage.rs`（在 `list_sessions` 方法后、trait 闭合 `}` 前插入）
...
```rust
// crates/memory-center-core/src/storage.rs line 43
...
```bash
git add crates/memory-center-core/src/storage.rs
...
**Files:**
- Modify: `crates/memory-center-core/src/storage.rs`（在 LocalStorage 的 `impl Storage for LocalStorage` 块中，紧接 `write_project_memory` 方法后）
- Test: `crates/memory-center-core/tests/session_meta_local.rs`（新增）

- [ ] **Step 1: 写失败测试**

新建 `crates/memory-center-core/tests/session_meta_local.rs`：
...
Run: `cargo test -p memory-center-core --test session_meta_local`
...
```bash
git add crates/memory-center-core/src/storage.rs crates/memory-center-core/tests/session_meta_local.rs
...
**Files:**
- Modify: `crates/memory-center-core/src/sqlite.rs`（INIT_SQL 新增表 + impl Storage 新增 2 方法）
- Test: `crates/memory-center-core/tests/session_meta_sqlite.rs`（新增）

- [ ] **Step 1: 写失败测试**

新建 `crates/memory-center-core/tests/session_meta_sqlite.rs`：
...
Run: `cargo test -p memory-center-core --test session_meta_sqlite`
Expected: FAIL — `test_sqlite_write_then_read_session_meta` 失败（默认实现 Ok(None)）

- [ ] **Step 3: 在 INIT_SQL 末尾新增 session_meta 表**

在 `crates/memory-center-core/src/sqlite.rs` 的 `INIT_SQL` 常量末尾（约 line 113，`CREATE INDEX IF NOT EXISTS idx_hooks_memory ...` 后，闭合 `"#;` 前）追加：
...
修改 `crates/memory-center-core/src/sqlite.rs` line 35 的导入：
...
```bash
git add crates/memory-center-core/src/sqlite.rs crates/memory-center-core/tests/session_meta_sqlite.rs
...
**Files:**
- Modify: `crates/memory-center-core/src/cache.rs`（impl Storage 块中新增 2 方法）

**理由**：CachedStorage 装饰任意 Storage 后端，若走 trait 默认实现（Ok(None)/Ok(())），包装 LocalStorage/SqliteStorage 时 session_meta 会失效。必须透传到 inner。

- [ ] **Step 1: 写失败测试**

新建 `crates/memory-center-core/tests/session_meta_cached.rs`：
...
```bash
git add crates/memory-center-core/src/cache.rs crates/memory-center-core/tests/session_meta_cached.rs
...
**Files:**
- Modify: `crates/memory-center-presets/src/builder.rs`（新增 scenario_to_str + 扩展 scenario_from_str）
...
在 `crates/memory-center-presets/src/builder.rs` 的 `#[cfg(test)] mod tests` 块末尾（最后一个 `}` 前）追加：
...
Run: `cargo test -p memory-center-presets --lib scenario_to_str`
...
Run: `cargo test -p memory-center-presets --lib scenario_`
Expected: PASS — 全部 6 个新测试通过，原有测试不受影响

- [ ] **Step 6: 提交**

```bash
git add crates/memory-center-presets/src/builder.rs
...
**Files:**
- Create: `crates/memory-center-presets/src/scenario_detect.rs`
- Modify: `crates/memory-center-presets/Cargo.toml`（新增 memory-center-llm 依赖，本任务先加 memory-center-core 依赖）

- [ ] **Step 1: 在 Cargo.toml 中新增依赖**

修改 `crates/memory-center-presets/Cargo.toml`，在 `[dependencies]` 末尾追加：

```toml
memory-center-llm = { path = "../memory-center-llm", version = "0.1.0" }
```
...
memory-center-core = { path = "../memory-center-core", version = "0.1.0" }
memory-center-models = { path = "../memory-center-models", version = "0.1.0" }
memory-center-scenarios = { path = "../memory-center-scenarios", version = "0.1.0" }
memory-center-windows = { path = "../memory-center-windows", version = "0.1.0" }
memory-center-agents = { path = "../memory-center-agents", version = "0.1.0" }
memory-center-skills = { path = "../memory-center-skills", version = "0.1.0" }
memory-center-llm = { path = "../memory-center-llm", version = "0.1.0" }
```

- [ ] **Step 2: 创建 scenario_detect.rs 文件骨架**

创建 `crates/memory-center-presets/src/scenario_detect.rs`：
...
修改 `crates/memory-center-presets/src/lib.rs`（line 79-86），在 `pub mod linkage;` 后追加 `pub mod scenario_detect;`：
...
Run: `cargo test -p memory-center-presets --lib scenario_detect::tests::keyword_detect`
Expected: PASS — 全部 11 个 KeywordScenarioDetector 测试通过

- [ ] **Step 5: 提交**

```bash
git add crates/memory-center-presets/Cargo.toml crates/memory-center-presets/src/scenario_detect.rs crates/memory-center-presets/src/lib.rs
...
**Files:**
- Modify: `crates/memory-center-presets/src/scenario_detect.rs`（在 KeywordScenarioDetector 后追加）

- [ ] **Step 1: 在 scenario_detect.rs 顶部新增 use 语句**

修改 `crates/memory-center-presets/src/scenario_detect.rs` 的 use 段（line 50 附近）：
...
在 `crates/memory-center-presets/src/scenario_detect.rs` 的 `#[cfg(test)] mod tests` 块末尾（最后一个 `}` 前）追加：
...
修改 `crates/memory-center-presets/src/scenario_detect.rs` 测试模块的 use 段（约 line 142）：
...
Run: `cargo test -p memory-center-presets --lib scenario_detect::tests::http_`
Expected: PASS — 全部 10 个 HttpScenarioDetector 测试通过

- [ ] **Step 6: 提交**

```bash
git add crates/memory-center-presets/src/scenario_detect.rs
...
**Files:**
- Modify: `crates/memory-center-presets/src/scenario_detect.rs`（在 HttpScenarioDetector 后追加）
...
在 `crates/memory-center-presets/src/scenario_detect.rs` 测试模块末尾追加：
...
修改 `crates/memory-center-presets/Cargo.toml`，在文件末尾追加：
...
Run: `cargo test -p memory-center-presets --lib scenario_detect`
Expected: PASS — 全部场景识别测试通过（KeywordScenarioDetector + HttpScenarioDetector + HybridScenarioDetector + resolve_effective_scenario）

- [ ] **Step 6: 提交**

```bash
git add crates/memory-center-presets/src/scenario_detect.rs crates/memory-center-presets/Cargo.toml
...
**Files:**
- Modify: `crates/memory-center-presets/src/lib.rs`

- [ ] **Step 1: 更新 lib.rs 的导出**

修改 `crates/memory-center-presets/src/lib.rs`（line 79-87），最终内容：
...
- [ ] **Step 2: 验证 memory-center-presets 编译**

Run: `cargo build -p memory-center-presets`
Expected: 编译通过

- [ ] **Step 3: 提交**

```bash
git add crates/memory-center-presets/src/lib.rs
git commit -m "feat(presets): lib.rs 重导出场景识别 API (v2.33)"
```

---

## Task 10: MCP build_scenario_detector + 注入 MemoryCenterMcp

**Files:**
- Modify: `crates/memory-center-mcp/src/main.rs`（新增 `build_scenario_detector` 函数 + 注入 MemoryCenterMcp）
- Modify: `crates/memory-center-mcp/src/lib.rs`（MemoryCenterMcp 新增 `scenario_detector` 字段 + `with_scenario_detector` 链式方法）

- [ ] **Step 1: 在 memory-center-mcp/src/lib.rs 中新增 scenario_detector 字段**

修改 `crates/memory-center-mcp/src/lib.rs` line 60-102 的 MemoryCenterMcp 结构体定义。在 `summary_generator` 字段后、`combined_profile` 字段前插入 `scenario_detector` 字段：

```rust
// crates/memory-center-mcp/src/lib.rs line 60
...
修改 `crates/memory-center-mcp/src/lib.rs` 的 `new()` 方法（line 108-117）和 `with_conflict_detector()` 方法（line 128-140），都加入 `scenario_detector: None,`：
...
- [ ] **Step 4: 验证 memory-center-mcp 编译**

Run: `cargo build -p memory-center-mcp`
...
修改 `crates/memory-center-mcp/src/main.rs`，在 `build_summary_generator` 函数后（约 line 207 附近）追加：

```rust
/// 从环境变量构造场景识别器（v2.33 新增）
///
/// 复用 `MEMORY_CENTER_DETECTOR_*` 环境变量（与冲突检测器、摘要生成器共享 LLM 配置）：
///
/// - 配置了 `MEMORY_CENTER_DETECTOR_API_URL` + `API_KEY`：
...
fn build_scenario_detector() -> Arc<memory_center_presets::HybridScenarioDetector> {
    use memory_center_llm::LlmDetectorConfig;
    use memory_center_presets::scenario_detect::{HttpScenarioDetector, HybridScenarioDetector};
...
修改 `crates/memory-center-mcp/src/main.rs` line 67-70 的导入：

```rust
use memory_center_presets::{
...
- [ ] **Step 7: 在 main() 中调用 build_scenario_detector 并注入 MemoryCenterMcp**

用 Grep 找到 main.rs 中 `MemoryCenterMcp::with_conflict_detector` 或 `MemoryCenterMcp::new` 的调用位置（通常在 main 函数中）。在调用链中追加 `.with_scenario_detector(Some(build_scenario_detector()))`：

```rust
// 假设原代码大致如下（根据实际 main() 调整）：
let mcp = MemoryCenterMcp::with_conflict_detector(storage_root.clone(), conflict_detector)
...
```bash
git add crates/memory-center-mcp/src/lib.rs crates/memory-center-mcp/src/main.rs
git commit -m "feat(mcp): MemoryCenterMcp 注入 scenario_detector + build_scenario_detector (v2.33)"
```
...
**Files:**
- Modify: `crates/memory-center-mcp/src/lib.rs`（archive handler 中插入识别调用，约 line 728-750 附近）

- [ ] **Step 1: 在 archive handler 中插入场景识别调用**

定位到 `crates/memory-center-mcp/src/lib.rs` 中 archive 方法（line 692）的 `let (archive_threshold, summary_template) = if let Some(preset_req) = &params.preset { ... }` 块（约 line 728-749）。
...
修改 `crates/memory-center-mcp/src/lib.rs` 顶部导入（如有需要）：

```rust
// 若尚未导入 memory_center_agents::AgentFamily，添加：
use memory_center_agents::AgentFamily;
```

实际上 archive handler 中用 `memory_center_agents::AgentFamily::Custom(...)` 全限定路径，可不导入。
...
新建 `crates/memory-center-mcp/tests/scenario_detect_integration.rs`：
...
Run: `cargo test -p memory-center-mcp --test scenario_detect_integration`
...
```bash
git add crates/memory-center-mcp/src/lib.rs crates/memory-center-mcp/tests/scenario_detect_integration.rs
...
**Files:**
- Modify: `crates/memory-center-server/src/handlers.rs`（如 server 端有 archive handler）

**说明**：spec 提到 server 端 archive handler 也需同步调用 `resolve_effective_scenario`。如果 server 端的 archive 走的是与 MCP 完全独立的路径，需同样注入 HybridScenarioDetector；如果 server 端复用 memory-center-mcp 的逻辑（如通过 MemoryCenterMcp 内部调用），则本任务可跳过。

- [ ] **Step 1: 检查 server 端是否有独立 archive handler**

Run: `cargo build -p memory-center-server`（确认 server crate 存在）

用 Grep 搜索 `crates/memory-center-server/src` 中的 `archive` handler：

```
pattern: "async fn archive|build_from_strings"
path: crates/memory-center-server/src
```
...
```bash
git add crates/memory-center-server/
...
启动 MCP server（配置 `MEMORY_CENTER_DETECTOR_API_URL` 和 `MEMORY_CENTER_DETECTOR_API_KEY`），通过 LLM 调用 archive 工具，传入一段写作类对话（如"帮我写一篇文章"），观察日志：
...
更新 `c:\Users\LINGTIAN303\.trae-cn\memory\projects\-d----AI-MemoryCenter\project_memory.md` 的 `task_state` 章节：
