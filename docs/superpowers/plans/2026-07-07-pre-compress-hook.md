...
| 文件 | 职责 | 改动 |
|------|------|------|
| `crates/memory-center-core/src/model.rs` | IndexHook 定义 | 新增 archive_reason + raw_context_path 字段 |
| `crates/memory-center-core/src/storage.rs` | Storage trait + LocalStorage | trait 新增 3 方法(默认实现返回 Error) + LocalStorage 实现 |
| `crates/memory-center-core/src/sqlite.rs` | SqliteStorage | 新增 raw_contexts 表 + 实现 3 方法 + ALTER TABLE |
| `crates/memory-center-core/src/cache.rs` | CachedStorage | 透传 3 方法 |
| `crates/memory-center-core/src/lib.rs` | crate 入口 | 模块导出 context_parser |
| `crates/memory-center-mcp/src/lib.rs` | MCP 工具注册 | 新增 pre_compress_hook 工具 + PreCompressResult |
| `crates/memory-center-server/src/handlers.rs` | HTTP handler | 新增 pre_compress_handler + 路由 |
| `crates/memory-center-server/tests/http_integration.rs` | HTTP 集成测试 | 新增 3 个 pre-compress 测试 |

### 新增文件

| 文件 | 职责 |
|------|------|
| `crates/memory-center-core/src/context_parser.rs` | full_context 字符串解析器(JSON / 分隔符识别) |
| `crates/memory-center-mcp/tests/pre_compress_integration.rs` | MCP 集成测试 |
...
**Files:**
- Modify: `crates/memory-center-core/src/model.rs:366-390`
- Test: `crates/memory-center-core/src/model.rs` (同文件内 #[cfg(test)] 模块)

- [ ] **Step 1: 写失败测试 — 向后兼容反序列化**

在 `crates/memory-center-core/src/model.rs` 文件末尾的 `#[cfg(test)]` 模块中(若无则在文件末尾新建)新增测试:
...
```bash
cargo test -p memory-center-core --lib v2_34_tests
```
Expected: 编译失败,`no field archive_reason on type IndexHook`

- [ ] **Step 3: 修改 IndexHook 添加新字段**

在 `crates/memory-center-core/src/model.rs:366` 的 `IndexHook` 结构体中,在 `file_status` 字段之后添加:
...
```bash
cargo build -p memory-center-core
```
...
```bash
git add crates/memory-center-core/src/model.rs
...
**Files:**
- Modify: `crates/memory-center-core/src/storage.rs:97` (trait 定义)

- [ ] **Step 1: 在 Storage trait 末尾新增 3 方法**

在 `crates/memory-center-core/src/storage.rs` 的 `pub trait Storage: Send + Sync { ... }` 块末尾(`}` 之前)新增:
...
**Files:**
- Modify: `crates/memory-center-core/src/storage.rs` (LocalStorage impl 块,约 795 行)
- Test: `crates/memory-center-core/src/storage.rs` (同文件 test 模块)
...
```bash
cargo test -p memory-center-core --lib v2_34_raw_context_tests
```
...
**Files:**
- Modify: `crates/memory-center-core/src/sqlite.rs`
- Test: `crates/memory-center-core/src/sqlite.rs` (test 模块)
...
```bash
cargo test -p memory-center-core --lib sqlite::v2_34_raw_context_tests
```
...
```bash
git add crates/memory-center-core/src/sqlite.rs
...
**Files:**
- Modify: `crates/memory-center-core/src/cache.rs`
...
```bash
git add crates/memory-center-core/src/cache.rs
...
**Files:**
- Create: `crates/memory-center-core/src/context_parser.rs`
- Modify: `crates/memory-center-core/src/lib.rs` (导出模块)

- [ ] **Step 1: 写失败测试**

创建 `crates/memory-center-core/src/context_parser.rs`:
...
在 `crates/memory-center-core/src/lib.rs` 中(约第 50-64 行的 `pub mod` 块)添加:
...
```bash
cargo test -p memory-center-core --lib context_parser
```
Expected: PASS(8 个测试)
...
```bash
git add crates/memory-center-core/src/context_parser.rs crates/memory-center-core/src/lib.rs
...
**Files:**
- Modify: `crates/memory-center-mcp/src/lib.rs`

- [ ] **Step 1: 新增 PreCompressResult 结构**

在 `crates/memory-center-mcp/src/lib.rs` 中(在 `ArchiveResult` 附近)新增:
...
在 `impl MemoryCenterMcp` 块中(在 `archive` 方法之后)新增:
...
"#)]
    async fn pre_compress_hook(
        ...
        // 4. 尝试解析 turns
        let parse_result = memory_center_core::context_parser::parse_context(&full_context);
...
```rust
    ...
    async fn archive_parsed_turns(
        ...
        session_id: &str,
        hook_id: &str,
        turns: Vec<memory_center_models::Turn>,
        ...
        let config = if let Some(threshold) = archive_threshold {
            memory_center_core::archive::ArchiveConfig::default()
        ...
        let mut archiver = memory_center_core::archive::Archiver::new(config, storage, session_id, None);
        ...
        if let Some(snapshot_params) = task_state_snapshot {
            let snapshot = memory_center_models::TaskStateSnapshot {
...
```bash
cargo build -p memory-center-mcp
```
...
```bash
git add crates/memory-center-mcp/src/lib.rs
...
**Files:**
- Create: `crates/memory-center-mcp/tests/pre_compress_integration.rs`

- [ ] **Step 1: 写集成测试**

创建 `crates/memory-center-mcp/tests/pre_compress_integration.rs`:

```rust
use memory_center_mcp::MemoryCenterMcp;
use tempfile::TempDir;

async fn make_mcp() -> (MemoryCenterMcp, TempDir) {
    let tmp = TempDir::new().unwrap();
    let mcp = MemoryCenterMcp::builder()
        .with_root(tmp.path())
        .build();
    (mcp, tmp)
}
...
```bash
cargo test -p memory-center-mcp --test pre_compress_integration
```
Expected: PASS

注意: `MemoryCenterMcp::builder().with_root().build()` 构造方式需与现有测试一致(检查 `crates/memory-center-mcp/tests/` 下其他测试的构造方式)。

- [ ] **Step 3: Commit**

```bash
git add crates/memory-center-mcp/tests/pre_compress_integration.rs
...
**Files:**
- Modify: `crates/memory-center-server/src/handlers.rs`
- Modify: `crates/memory-center-server/src/lib.rs` (路由注册,若路由在 lib.rs)

- [ ] **Step 1: 新增 PreCompressRequest 结构**

在 `crates/memory-center-server/src/handlers.rs` 中(在 `ArchiveRequest` 附近)新增:
...
```bash
cargo build -p memory-center-server
```
Expected: PASS
...
```bash
git add crates/memory-center-server/src/handlers.rs crates/memory-center-server/src/lib.rs
...
**Files:**
- Modify: `crates/memory-center-server/tests/http_integration.rs`
...
```bash
cargo test -p memory-center-server --test http_integration test_http_pre_compress
```
Expected: PASS
...
```bash
git add crates/memory-center-server/tests/http_integration.rs
...
**Files:**
- Modify: `AGENTS.md`
- Modify: `.trae/rules/memory-center-archive.md`
- Modify: `.catpaw/rules/memory-center-archive.md`
...
- [ ] **Step 2: .trae/rules/memory-center-archive.md 同步更新**

在文件末尾新增「pre_compress_hook 调用时机」章节(同上内容)。

- [ ] **Step 3: .catpaw/rules/memory-center-archive.md 同步更新**
...
```bash
git add AGENTS.md .trae/rules/memory-center-archive.md .catpaw/rules/memory-center-archive.md
...
#### 变更
- `crates/memory-center-core/src/model.rs`:IndexHook 新增 2 字段
- `crates/memory-center-core/src/storage.rs`:Storage trait 新增 3 方法 + LocalStorage 实现
- `crates/memory-center-core/src/sqlite.rs`:SqliteStorage 迁移 + 实现 3 方法
- `crates/memory-center-core/src/cache.rs`:CachedStorage 透传
- `crates/memory-center-core/src/context_parser.rs`(新):上下文解析器
- `crates/memory-center-mcp/src/lib.rs`:新增 pre_compress_hook 工具
- `crates/memory-center-server/src/handlers.rs`:新增 POST /pre-compress 端点
...
1. **方法/字段名匹配**:Task 7 Step 3 的 `self.create_storage()` / `self.summary_generator` / `self.combined_profile()` 需与现有 MemoryCenterMcp 实现一致
2. **TaskStateSnapshotParams 字段名**:需与 `crates/memory-center-mcp/src/lib.rs:435` 实际定义一致
3. **MemoryCenterMcp 构造方式**:Task 8 的 `MemoryCenterMcp::builder().with_root().build()` 需与现有测试一致
