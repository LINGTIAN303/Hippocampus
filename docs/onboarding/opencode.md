...
> 适用版本：MemoryCenter v2.38+ / OpenCode（dev 分支，`packages/core` schema）
>
...
│  - prompt             - 何时调哪个工具      - time_compacting│
│  - archive            - session_id 约定       字段变化      │
...
Compaction.Started
  ↓
time_compacting = ts  →   检测到字段从 NULL → ts
  ↓                        （记录为"压缩中"）
LLM 生成摘要
  ↓
Compaction.Ended
  ↓
time_compacting = NULL →   检测到字段从 ts → NULL    →  POST /pre-compress
                          （判定压缩完成）               归档完整上下文
```
...
触发压缩（/compact 手动 或 compactIfNeeded 自动）
    ↓
【路径三】sidecar 检测 time_compacting 变化 → 自动调 pre-compress 归档完整上下文
    ↓
...
- 用 SQLite 工具检查 `session` 表的 `time_compacting` 字段是否有变化
