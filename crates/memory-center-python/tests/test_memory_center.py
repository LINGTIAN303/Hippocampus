"""MemoryCenter Python 绑定全链路集成测试

测试 5 个核心方法 + 上下文管理器 + 错误处理 + 会话隔离。
运行方式：在虚拟环境中 `pytest tests/test_MemoryCenter.py -v`
"""
...
def test_module_version():
    """测试模块版本号"""
    assert memory_center_python.version() == "0.1.0"


def test_module_operations():
    """测试模块操作列表"""
    ops = memory_center_python.operations()
...
def test_construct(temp_storage):
    """测试构造"""
    hp = MemoryCenter(temp_storage, "sess-1")
    assert hp is not None
    repr_str = repr(hp)
    assert "MemoryCenter" in repr_str
    assert "sess-1" in repr_str


def test_context_manager(temp_storage):
    """测试上下文管理器"""
    with MemoryCenter(temp_storage, "sess-ctx") as hp:
        assert hp is not None
        # 在上下文中执行操作
...
def test_construct_invalid_path(tmp_path):
    ...
    with pytest.raises(ValueError, match="创建存储目录失败"):
        MemoryCenter(str(file_path), "sess-x")
...
def test_archive_success(temp_storage):
    """测试归档成功"""
    hp = MemoryCenter(temp_storage, "sess-archive")
...
def test_archive_empty_turns(temp_storage):
    """测试空 turns 报错"""
    hp = MemoryCenter(temp_storage, "sess-empty")
...
def test_archive_with_project_id(temp_storage):
    """测试带 project_id 归档"""
    hp = MemoryCenter(temp_storage, "sess-proj", project_id="proj-a")
...
def test_summaries_empty(temp_storage):
    """测试空会话 summaries"""
    hp = MemoryCenter(temp_storage, "never-exist")
...
def test_summaries_after_archive(temp_storage):
    """测试归档后 summaries"""
    hp = MemoryCenter(temp_storage, "sess-summ")
    # 归档 2 次
...
def test_retrieve_full_chain(temp_storage):
    """测试 retrieve 全链路"""
    hp = MemoryCenter(temp_storage, "sess-ret")
...
def test_retrieve_nonexistent(temp_storage):
    """测试检索不存在的 hook_id"""
    hp = MemoryCenter(temp_storage, "sess-nope")
...
def test_prompt_empty(temp_storage):
    """测试空会话 prompt"""
    hp = MemoryCenter(temp_storage, "sess-prompt-empty")
    prompt = hp.prompt()
    assert prompt == ""
...
def test_prompt_with_memory(temp_storage):
    """测试有记忆时 prompt"""
    hp = MemoryCenter(temp_storage, "sess-prompt")
    hp.archive(make_turns(2))
...
def test_compaction_invalid_period(temp_storage):
    """测试无效 period"""
    hp = MemoryCenter(temp_storage, "sess-comp")
...
def test_compaction_weekly_without_daily(temp_storage):
    ...
    hp = MemoryCenter(temp_storage, "sess-weekly")
    with pytest.raises(ValueError, match="周期任务失败"):
...
def test_compaction_full_workflow(temp_storage):
    """测试周期任务完整工作流"""
    hp = MemoryCenter(temp_storage, "sess-fw")
...
def test_session_isolation(temp_storage):
    """测试会话隔离"""
    hp_a = MemoryCenter(temp_storage, "sess-iso-a")
    hp_a.archive(make_turns(2))

    hp_b = MemoryCenter(temp_storage, "sess-iso-b")
...
def test_project_id_isolation(temp_storage):
    """测试项目隔离"""
    hp_a = MemoryCenter(temp_storage, "sess-proj", project_id="proj-a")
    hp_a.archive(make_turns(2))

    hp_b = MemoryCenter(temp_storage, "sess-proj", project_id="proj-b")
    summaries_b = hp_b.summaries()
...
def test_full_agent_workflow(temp_storage):
    ...
    with MemoryCenter(temp_storage, "agent-full", project_id="demo") as hp:
        # 1. 归档
