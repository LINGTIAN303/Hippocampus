...
// TestNewMemoryCenter 验证创建实例成功。
func TestNewMemoryCenter(t *testing.T) {
	storage := tempStorage(t)
	hp, err := New(storage, "sess-1", "")
	if err != nil {
		t.Fatalf("创建 MemoryCenter 失败: %v", err)
	}
	defer hp.Close()
...
// TestNewMemoryCenterWithProjectID 验证带 project_id 创建实例。
func TestNewMemoryCenterWithProjectID(t *testing.T) {
	storage := tempStorage(t)
...
// TestMemoryCenterString 验证 String() 包含 "MemoryCenter"。
func TestMemoryCenterString(t *testing.T) {
	storage := tempStorage(t)
	hp, _ := New(storage, "sess-str", "proj-x")
	defer hp.Close()

	s := hp.String()
	if !strings.Contains(s, "MemoryCenter") {
		t.Errorf("String 应包含 MemoryCenter，实际: %q", s)
	}
}

// TestMemoryCenterClose 验证 Close 幂等（多次调用不 panic）。
func TestMemoryCenterClose(t *testing.T) {
	storage := tempStorage(t)
...
//
// 与 Node 测试 test_memory_center_archive_full_workflow 对齐。
