...
//! ## 模块组织（v2.35 WASM 拆分后）
//!
//! - **纯逻辑层**（来自 `MemoryCenter-core-logic` crate，WASM 兼容）：
...
// ============================================================================
// IO 实现层模块（MemoryCenter-core 独有，依赖原生 fs/SQLite）
// ============================================================================
...
// ============================================================================
// 从 MemoryCenter-core-logic 重导出纯逻辑模块（v2.35 WASM 拆分）
// ============================================================================
//
// 所有纯逻辑模块的源文件已迁移到 MemoryCenter-core-logic/src/ 中，
// MemoryCenter-core/src/ 中已删除这些文件，lib.rs 通过 `pub use` 重导出。
// 这样 `crate::model::*` / `crate::bm25::*` / `crate::archive::*` 等在
// MemoryCenter-core 中指向 core-logic 的版本，与 Storage trait 中的
// `crate::model::*` 类型一致。

/// 归档/冻结逻辑（达到阈值时将上下文冻结为记忆文件）
pub use memory_center_core_logic::archive;
/// BM25 关键词检索（jieba-rs 中文分词 + 倒排索引）
pub use memory_center_core_logic::bm25;
/// 周期任务（周去重合并 / 月评分淘汰）
pub use memory_center_core_logic::compact;
/// 记忆冲突检测（ConflictDetector trait + NoopDetector）
pub use memory_center_core_logic::conflict;
/// 上下文字符串解析器（v2.34，pre_compress_hook 用）
pub use memory_center_core_logic::context_parser;
/// LLM 生成器 trait（AnchorGenerator / SummaryGenerator，v2.16 IMP-05/10）
pub use memory_center_core_logic::generate;
/// 启发式冲突检测器（HeuristicDetector，反义词词典 + 三维度检测）
pub use memory_center_core_logic::heuristic;
/// 混合检索器（HybridRetriever + RRF 融合 + 降级策略）
pub use memory_center_core_logic::hybrid;
pub use memory_center_core_logic::migrator;
pub use memory_center_core_logic::model;
/// 检索机制（摘要钩子注入 + tool 主动检索）
pub use memory_center_core_logic::retrieve;
pub use memory_center_core_logic::score;
/// 语义检索（Embedder / KeywordSearcher / VectorIndex / SemanticRetriever trait + RRF 融合）
pub use memory_center_core_logic::semantic;
/// 序列化格式（JSON / MessagePack 双格式支持）
pub use memory_center_core_logic::serialization;
/// 向量索引（InMemoryVectorIndex + cosine_similarity）
pub use memory_center_core_logic::vector;

// ============================================================================
// 从 core-logic 重导出 Error / Result（保持 `crate::Error` / `crate::Result` 可用）
// ============================================================================
//
// MemoryCenter-core 不再自定义 Error / Result，而是从 core-logic 重导出。
// 这样 Storage trait 中的 `crate::Result` / `crate::Error`（定义在 core-logic）
// 与 MemoryCenter-core 中 LocalStorage impl 的 `crate::Result` / `crate::Error`
// 指向同一类型，避免类型不匹配。

/// Crate 级错误类型（从 core-logic 重导出）
pub use memory_center_core_logic::Error;

/// Crate 级结果别名（从 core-logic 重导出）
pub use memory_center_core_logic::Result;

// ============================================================================
// 显式重导出 Storage trait 和 SessionMeta
// ============================================================================
//
// 保持 `use memory_center_core::storage::Storage` / `use memory_center_core::Storage` 可用。
// MemoryCenter-core/src/storage.rs 中只有 LocalStorage 实现，trait 来自 core-logic。

/// Storage trait（存储后端接口，来自 core-logic）
pub use memory_center_core_logic::storage::Storage;

/// SessionMeta（session 元数据，来自 core-logic）
pub use memory_center_core_logic::storage::SessionMeta;
