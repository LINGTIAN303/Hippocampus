...
use memory_center_core::model::IndexHook;
use memory_center_core::semantic::{Embedder, KeywordSearcher, VectorIndex};
use std::sync::Arc;
...
mod tests {
    ...
    use memory_center_core::model::{ArchivePeriod, Summary, Tag};
    ...
    async fn test_index_hook_embedder_failure_skips_vector() {
        use memory_center_core::semantic::KeywordSearcher;
        ...
        impl Embedder for FailEmbedder {
            ...
            async fn embed(&self, _text: &str) -> memory_center_core::Result<Vec<f32>> {
                Err(memory_center_core::Error::Storage("mock failure".into()))
            }
        }
