...
use memory_center_core::bm25::Bm25Searcher;
use memory_center_core::hybrid::{HybridRetriever, KeywordOnlyRetriever};
use memory_center_core::model::{ArchivePeriod, IndexHook};
use memory_center_core::semantic::{
    Embedder, KeywordSearcher, SearchHit, SemanticRetriever, VectorIndex,
};
use memory_center_core::storage::Storage;
use memory_center_core::vector::InMemoryVectorIndex;
...
impl SessionSearchRouter {
    ...
    ) -> memory_center_core::Result<usize> {
        let storage = self.storage.as_ref().ok_or_else(|| {
            memory_center_core::Error::Storage("storage 未注入，无法重建索引".into())
        })?;
...
mod tests {
    use super::*;
    use memory_center_core::model::{ArchivePeriod, IndexDocument, Summary, Tag};
    use chrono::Utc;
    use uuid::Uuid;
    ...
    impl Embedder for MockEmbedder {
        ...
        async fn embed(&self, text: &str) -> memory_center_core::Result<Vec<f32>> {
    ...
    fn make_hook(title: &str, key_facts: Vec<String>) -> IndexHook {
        IndexHook {
            ...
            token_count: 100,
            file_status: memory_center_core::model::FileStatus::Normal,
            // v2.34：测试辅助函数默认 None
            archive_reason: None,
    ...
    impl Storage for MockStorage {
        async fn write_memory(
            &self,
            _file: &memory_center_core::model::MemoryFile,
        ) -> memory_center_core::Result<String> {
        ...
        ) -> memory_center_core::Result<Option<IndexDocument>> {
        ...
        async fn list_memories(
            ...
            _project_id: Option<&str>,
            _period: ArchivePeriod,
        ) -> memory_center_core::Result<Vec<String>> {
            Ok(Vec::new())
        }
    }
    ...
    async fn test_search_with_rebuild_uses_embed_batch() {
        ...
        impl Embedder for CountingEmbedder {
            ...
            async fn embed(&self, _text: &str) -> memory_center_core::Result<Vec<f32>> {
                self.embed_count
            ...
            async fn embed_batch(
                &self,
                texts: &[&str],
            ) -> memory_center_core::Result<Vec<Vec<f32>>> {
