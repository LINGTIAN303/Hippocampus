//! # MemoryCenter LLM 客户端组件库（v2.12 新增）
//!
//! 隔离 LLM HTTP 客户端依赖（reqwest），供 `memory-center-server` 和 `memory-center-mcp` 共享复用。
//!
//! ## 架构定位
//!
...
//! ## v2.12 迁移说明
//!
//! 本 crate 从 `memory-center-server` 下沉而来，原 `server/llm_detector.rs` / `server/embedding.rs` /
//! `server/llm.rs` 已迁移到此处。`memory-center-server` 通过 re-export 保持向后兼容。
