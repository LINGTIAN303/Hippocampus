...
//!
//! 基于 [`memory_center_core::semantic::Embedder`] trait 的 HTTP 实现，
...
impl EmbedderConfig {
    /// 环境变量前缀
    pub const ENV_PREFIX: &'static str = "MEMORY_CENTER_EMBEDDER";

    /// 从环境变量构造配置（v2.13 新增）
    ///
    /// 读取以下环境变量（前缀 `MEMORY_CENTER_EMBEDDER_`）：
    ///
    /// | 环境变量 | 字段 | 默认值 |
    /// |---------|------|--------|
...
impl Embedder for HttpEmbedder {
    ...
    async fn embed(&self, text: &str) -> memory_center_core::Result<Vec<f32>> {
        ...
        let resp = self
            ...
            .await
            .map_err(|e| {
                memory_center_core::Error::Storage(format!("Embedding API 请求失败: {}", e))
            })?;
        ...
        if !resp.status().is_success() {
            ...
            return Err(memory_center_core::Error::Storage(format!(
                "Embedding API 错误: {} {}",
                status, body
            )));
        }
        ...
        let resp_json: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| memory_center_core::Error::Storage(format!("Embedding API 响应解析失败: {}", e)))?;
        ...
        let embedding = resp_json
            ...
            .ok_or_else(|| {
                memory_center_core::Error::Storage("Embedding API 响应格式错误：缺少 data[0].embedding".into())
            })?;
    ...
    async fn embed_batch(&self, texts: &[&str]) -> memory_center_core::Result<Vec<Vec<f32>>> {
        // 未配置时降级为逐条调用（触发降级）
        if self.config.api_url.is_empty() {
            return Err(memory_center_core::Error::Storage(
                "Embedder 未配置 api_url".into(),
        ...
        let resp = self
            ...
            .await
            .map_err(|e| {
                memory_center_core::Error::Storage(format!("Embedding API 批量请求失败: {}", e))
            })?;
        ...
        let data = resp_json
            ...
            .ok_or_else(|| {
                memory_center_core::Error::Storage("Embedding API 响应格式错误：缺少 data 数组".into())
            })?;
        ...
        for item in data {
            ...
            let embedding = item
                ...
                .and_then(|e| e.as_array())
                .ok_or_else(|| {
                    memory_center_core::Error::Storage("Embedding API 响应格式错误：缺少 embedding".into())
                })?;
            let vector: Vec<f32> = embedding
                .iter()
                .map(|v| v.as_f64().unwrap_or(0.0) as f32)
