...
/**
 * MemoryCenter Java 绑定的单元测试（v2.15-c 新增）。
 *
 * <p>共 12 个测试，覆盖版本/操作列表/创建/归档/检索/摘要/Prompt/周期任务/全链路，
 * 与 Go 绑定（{@code crates/MemoryCenter-go/MEMORY_CENTER_test.go}）对齐。
 *
 * <p>运行测试前需确保 MemoryCenter 动态库已构建（{@code cargo build --release -p memory-center-ffi}），
 * 且库路径已通过 pom.xml 的 surefire 插件配置（{@code jna.library.path}）注入。
 */
@DisplayName("MemoryCenter Java 绑定测试")
class MemoryCenterTest {
    ...
    void testVersion() {
        System.out.println("版本号: " + MemoryCenter.VERSION);
        assertEquals("0.1.0", MemoryCenter.VERSION);
    }

    @Test
    @DisplayName("支持的操作列表应包含 5 个操作")
    void testOperations() {
        List<String> ops = MemoryCenter.OPERATIONS;
    ...
    void testNewMemoryCenter() {
        try (MemoryCenter hp = MemoryCenter.create(storagePath(), "session-1", null)) {
            assertNotNull(hp);
    ...
    void testNewMemoryCenterWithProjectId() {
        try (MemoryCenter hp = MemoryCenter.create(storagePath(), "session-2", "project-x")) {
            assertNotNull(hp);
        }
    }
    ...
    void testMemoryCenterToString() {
        try (MemoryCenter hp = MemoryCenter.create(storagePath(), "session-3", null)) {
            assertEquals("MemoryCenter(handle=valid)", hp.toString());
        }
    }

    @Test
    @DisplayName("close 后 toString 应显示 closed")
    void testMemoryCenterClose() {
        MemoryCenter hp = MemoryCenter.create(storagePath(), "session-4", null);
        hp.close();
        assertEquals("MemoryCenter(closed)", hp.toString());
        // 多次 close 应安全（幂等）
        hp.close();
    }
    ...
    void testArchiveEmptyJson() {
        try (MemoryCenter hp = MemoryCenter.create(storagePath(), "session-5", null)) {
    ...
    void testArchiveInvalidJson() {
        try (MemoryCenter hp = MemoryCenter.create(storagePath(), "session-6", null)) {
    ...
    void testRetrieveNonexistent() {
        try (MemoryCenter hp = MemoryCenter.create(storagePath(), "session-7", null)) {
    ...
    void testSummariesEmpty() {
        try (MemoryCenter hp = MemoryCenter.create(storagePath(), "session-8", null)) {
    ...
    void testCompactionInvalidPeriod() {
        try (MemoryCenter hp = MemoryCenter.create(storagePath(), "session-9", null)) {
    ...
    void testArchiveFullWorkflow() {
        try (MemoryCenter hp = MemoryCenter.create(storagePath(), "session-10", null)) {
            // 1. 归档
