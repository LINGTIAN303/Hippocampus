...
/**
 * MemoryCenter 记忆库的 Java 绑定（v2.15-c 新增）。
 *
 * <p>通过 JNA 包装 memory-center-ffi 的 C ABI，将 Rust 核心能力暴露为 Java API。
 *
 * <h3>架构</h3>
 * <ul>
 *   <li>Java ↔ JNA ↔ C ABI ↔ Rust（memory-center-ffi crate）</li>
 *   <li>JSON 中间转换：Java String ↔ JSON ↔ Rust structs（与 Python/Node/Go 绑定一致）</li>
 *   <li>同步 API：C ABI 内部通过 tokio block_on 执行异步方法</li>
 * </ul>
 *
 * <h3>依赖</h3>
 * <p>需先构建 MemoryCenter 动态库：
 * <pre>{@code
 * cargo build --release -p memory-center-ffi
 * }</pre>
 * <p>然后将动态库（memory_center.dll / libmemory_center.so / libmemory_center.dylib）
 * 放到 Java 可访问的路径：
 * <ul>
 ...
 * <h3>示例</h3>
 * <pre>{@code
 * try (MemoryCenter hp = MemoryCenter.create("./data", "session-1", null)) {
 ...
 */
public class MemoryCenter implements Closeable {

    /** 绑定版本号（与 Rust crate 版本同步）。 */
    public static final String VERSION = "0.1.0";

    /** 周级合并参数（对应 C 宏 MEMORY_CENTER_COMPACTION_WEEKLY）。 */
    public static final int COMPACTION_WEEKLY = MemoryCenterLibrary.MEMORY_CENTER_COMPACTION_WEEKLY;

    /** 月级评分淘汰参数（对应 C 宏 MEMORY_CENTER_COMPACTION_MONTHLY）。 */
    public static final int COMPACTION_MONTHLY = MemoryCenterLibrary.MEMORY_CENTER_COMPACTION_MONTHLY;
    ...
    private static final MemoryCenterLibrary LIB = MemoryCenterLibrary.INSTANCE;

    private Pointer handle;

    private MemoryCenter(Pointer handle) {
        this.handle = handle;
    }

    /**
     * 创建新的 MemoryCenter 实例。
     *
     * @param rootPath   存储根目录路径（自动创建）
     * @param sessionId  会话 ID（一个实例绑定一个会话）
     * @param projectId  项目 ID，传 null 表示无项目隔离
     * @return MemoryCenter 实例
     * @throws IllegalArgumentException 参数无效（空字符串）
     * @throws RuntimeException         runtime 创建失败（FFI 返回 null）
     */
    public static MemoryCenter create(String rootPath, String sessionId, String projectId) {
        ...
        Pointer h = LIB.memory_center_new(rootPath, sessionId, projectId);
        if (h == null) {
            throw new RuntimeException("创建 MemoryCenter 失败（参数无效或 runtime 创建失败）");
        }
        return new MemoryCenter(h);
    }

    /**
     * 释放 MemoryCenter 实例资源。
     *
    ...
    public String toString() {
        return handle != null ? "MemoryCenter(handle=valid)" : "MemoryCenter(closed)";
    }
    ...
    public String archive(String turnsJson) {
        ...
        Pointer result = LIB.memory_center_archive(handle, turnsJson);
    ...
    public String retrieve(String hookId) {
        ...
        Pointer result = LIB.memory_center_retrieve(handle, hookId);
        return resultToString(result);
    }
    ...
    public String summaries() {
        ensureOpen();
        Pointer result = LIB.memory_center_get_summaries(handle);
        return resultToString(result);
    }
    ...
    public String prompt() {
        ensureOpen();
        Pointer result = LIB.memory_center_render_prompt(handle);
        return resultToString(result);
    }
    ...
    public String compaction(String period) {
        ...
        Pointer result = LIB.memory_center_run_compaction(handle, periodCode);
        return resultToString(result);
    }
    ...
    private void ensureOpen() {
        if (handle == null) {
            throw new IllegalStateException("MemoryCenter 已关闭");
        }
    }

    /**
     * 将 C ABI 的 MemoryCenterResult 转为 Java String。
     *
    ...
    private static String resultToString(Pointer result) {
        ...
        try {
            if (!LIB.memory_center_is_ok(result)) {
                // 失败：读取错误消息
                Pointer cErr = LIB.memory_center_get_error(result);
                if (cErr != null) {
                    try {
                        throw new RuntimeException(cErr.getString(0, "UTF-8"));
                    } finally {
                        LIB.memory_center_free_string(cErr);
                    }
                }
            ...
            // 成功：读取数据
            Pointer cData = LIB.memory_center_get_data(result);
            if (cData != null) {
                try {
                    return cData.getString(0, "UTF-8");
                } finally {
                    LIB.memory_center_free_string(cData);
                }
            }
            ...
            LIB.memory_center_result_free(result);
        }
    }
}
