...
use memory_center_server::{create_router, AppState};
...
impl TestServer {
    ...
    async fn start_with_session_search(
        session_search: Option<std::sync::Arc<memory_center_server::SessionSearchRouter>>,
        conflict_detector: Option<
            std::sync::Arc<dyn memory_center_core::conflict::ConflictDetector>,
        >,
    ) -> Self {
...
async fn test_update_with_detector_detects_direct_contradiction() {
    ...
    let detector: std::sync::Arc<dyn memory_center_core::conflict::ConflictDetector> =
        std::sync::Arc::new(memory_center_core::heuristic::HeuristicDetector::new());
