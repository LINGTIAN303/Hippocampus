...
/// 从 Core 错误转换
impl From<memory_center_core::Error> for AppError {
    fn from(e: memory_center_core::Error) -> Self {
        match e {
            // 索引错误中含「未找到」「不存在」「已删除」视为 404
            // （如 hook_id 精确+前缀匹配均无结果、记忆文件已软删除）
            memory_center_core::Error::Index(msg)
            ...
            memory_center_core::Error::Storage(msg)
                if msg.contains("不存在") || msg.contains("读取记忆文件失败") =>
            {
                AppError::NotFound(msg)
            }
            // 序列化错误视为 400（客户端传了非法 JSON）
            memory_center_core::Error::Serialize(msg) => AppError::BadRequest(msg),
            // 其余视为内部错误
