...
//! per-session 识别实际依赖：
//!
//! - **Layer 1**（推荐）：`MEMORY_CENTER_PRESET_AGENT` 环境变量（服务器启动时设置）
//! - **Layer 3**：父进程名 / 环境变量前缀（HTTP 模式下父进程是 systemd/手动启动）
//! - **Layer 4**：降级 Custom（不注入 preset）
//!
//! **生产环境推荐**：在 systemd unit 或部署脚本中设置 `MEMORY_CENTER_PRESET_AGENT=trae`
//! （或 `claude_code` / `cursor` / `codex`），所有连接的 session 统一识别为该 family。
//! 这符合服务端持久化部署的常见模式。
//!
//! 未来若需真正的 per-session ClientInfo 识别，需在 `MemoryCenterMcp` 内部拦截
...
///
/// 每次创建新 session 时调用，构造一个完整的 `MemoryCenterMcp` 实例。
...
) -> impl Fn() -> Result<MemoryCenterMcp, std::io::Error> + Send + Sync + 'static {
    move || {
...
/// let app = memory_center_server::mcp::mount_mcp_route(app, &mcp_config);
/// ```
