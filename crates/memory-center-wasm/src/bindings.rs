...
//!
//! ```js
//! import { MemoryCenterCore, MemoryStorage } from "memory_center_wasm";
//!
//! const core = MemoryCenterCore.with_memory_storage(new MemoryStorage());
...
use memory_center_core_logic::archive::Archiver;
use memory_center_core_logic::model::{ArchiveConfig, ArchivePeriod, MessageTurn};
use memory_center_core_logic::storage::Storage;
use std::sync::Arc;
use wasm_bindgen::prelude::*;

/// MemoryCenterCore - JS 调用 MemoryCenter 主入口
///
...
impl MemoryCenterCore {
    /// 用 [`MemoryStorage`](crate::MemoryStorage) 构造 MemoryCenterCore
    ///
    /// 纯内存存储，重启丢失。适用于 demo / 测试 / 无状态计算。
    ///
    /// JS: `MemoryCenterCore.with_memory_storage(new MemoryStorage())`
    #[wasm_bindgen(static_method_of = MemoryCenterCore)]
    pub fn with_memory_storage(storage: crate::MemoryStorage) -> MemoryCenterCore {
        Self {
            storage: Arc::new(storage),
        }
    }

    /// 用 [`JsStorage`](crate::JsStorage) 构造 MemoryCenterCore
    ///
    /// JS 调用方实现存储后端（IndexedDB / Workers KV / 远程服务）。
    ///
    /// JS: `MemoryCenterCore.with_js_storage(new JsStorage(callbacks))`
    #[cfg(target_arch = "wasm32")]
    #[wasm_bindgen(static_method_of = MemoryCenterCore)]
    pub fn with_js_storage(storage: crate::JsStorage) -> MemoryCenterCore {
        Self {
            storage: Arc::new(storage),
        }
    }
