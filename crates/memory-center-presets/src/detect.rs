...
//! ```text
//! Layer 1: 显式声明（最高优先级）
//!   MEMORY_CENTER_PRESET_AGENT 环境变量
//!   → 直接使用，跳过自动识别
//!
//! Layer 2: MCP 协议信号
...
//!
//! ```rust,ignore
//! use memory_center_presets::detect::detect_agent_client;
//!
...
pub enum DetectionSource {
    /// Layer 1：环境变量 `MEMORY_CENTER_PRESET_AGENT` 显式声明
    ExplicitEnv,
...
/// Layer 1：从环境变量 `MEMORY_CENTER_PRESET_AGENT` 显式读取
///
...
fn detect_from_explicit_env() -> Option<DetectedAgent> {
    let value = std::env::var("MEMORY_CENTER_PRESET_AGENT").ok()?;
...
///
/// 1. 显式环境变量 `MEMORY_CENTER_PRESET_AGENT`
...
pub fn detect_agent_client(client_info_name: Option<&str>) -> DetectedAgent {
    ...
    // Layer 4: 未识别
    tracing::info!(
        "Agent 客户端未识别，降级为 Custom(\"unknown\")。建议设置 MEMORY_CENTER_PRESET_AGENT 环境变量"
    );
    DetectedAgent {
...
/// - 其他 → Daily（日常场景，保守归档）
///
/// 调用方可通过 `MEMORY_CENTER_PRESET_SCENARIO` 环境变量覆盖。
...
/// 从环境变量 `MEMORY_CENTER_PRESET_SCENARIO` 读取，未设置则按 agent 推导
///
/// 返回小写场景名（如 "coding" / "daily" / "writing"）
pub fn resolve_scenario_name(family: &AgentFamily) -> String {
    std::env::var("MEMORY_CENTER_PRESET_SCENARIO")
...
mod tests {
    ...
    fn test_detect_from_explicit_env_valid() {
        // 临时设置环境变量
        std::env::set_var("MEMORY_CENTER_PRESET_AGENT", "Trae");
        let result = detect_from_explicit_env();
        std::env::remove_var("MEMORY_CENTER_PRESET_AGENT");
    ...
    fn test_detect_from_explicit_env_invalid_value() {
        std::env::set_var("MEMORY_CENTER_PRESET_AGENT", "UnknownAgent");
        let result = detect_from_explicit_env();
        std::env::remove_var("MEMORY_CENTER_PRESET_AGENT");
    ...
    fn test_detect_agent_client_explicit_env_priority() {
        ...
        std::env::set_var("MEMORY_CENTER_PRESET_AGENT", "Cursor");
        let result = detect_from_explicit_env();
        std::env::remove_var("MEMORY_CENTER_PRESET_AGENT");
    ...
    fn test_resolve_scenario_name_from_env() {
        std::env::set_var("MEMORY_CENTER_PRESET_SCENARIO", "writing");
        let scenario = resolve_scenario_name(&AgentFamily::Trae);
        std::env::remove_var("MEMORY_CENTER_PRESET_SCENARIO");
