...
// v2.46：SidecarTurn / SidecarContent / SidecarToolCall / SidecarFileChange
// 已迁移到 memory-center-adapter crate，这里 re-export 保持外部引用路径不变。
// 这样 opencode_db.rs / main.rs 中的 `use crate::archive::{SidecarTurn, ...}`
// 无需修改。
pub use memory_center_adapter::{
    SidecarContent, SidecarFileChange, SidecarToolCall, SidecarTurn,
};
