//! # MemoryCenter FFI
//!
//! C ABI 动态库，将 [`memory_center_core`] 的核心能力暴露为 C 接口，
...
//! - **错误处理**：所有函数返回 `MemoryCenterResult*`，调用方负责释放
//!
//! ## 基本用法（伪代码）
//!
//! ```c
//! MemoryCenterHandle* h = memory_center_new("/path/to/store", "session-1", NULL);
//! MemoryCenterResult* r = memory_center_archive(h, turns_json);
//! if (memory_center_is_ok(r)) {
//!     char* summary = memory_center_get_data(r);
//!     // ...处理 summary...
//!     memory_center_free_string(summary);
//! }
...
use memory_center_core::model::{ArchiveConfig, MessageTurn};
...
pub struct MemoryCenterResult {
    is_ok: bool,
    error_message: Option<CString>,
    data: Option<CString>,
}

/// 周期任务返回的精简结果（用于 [`memory_center_run_compaction`]）
...
fn ok_result<T: serde::Serialize>(data: &T) -> *mut MemoryCenterResult {
    match serde_json::to_string(data) {
        Ok(json) => {
            // JSON 不会包含 null 字节，unwrap 安全
            let cstr = CString::new(json).unwrap_or_else(|_| CString::new("{}").unwrap());
            Box::into_raw(Box::new(MemoryCenterResult {
                is_ok: true,
                error_message: None,
                data: Some(cstr),
            }))
...
fn ok_result_str(s: &str) -> *mut MemoryCenterResult {
    match CString::new(s) {
        Ok(cstr) => Box::into_raw(Box::new(MemoryCenterResult {
            is_ok: true,
            error_message: None,
            data: Some(cstr),
        })),
...
/// 构造错误结果
fn err_result(msg: &str) -> *mut MemoryCenterResult {
    Box::into_raw(Box::new(MemoryCenterResult {
        is_ok: false,
...
/// 从 Core Error 构造错误结果
fn err_from_core(e: memory_center_core::Error) -> *mut MemoryCenterResult {
    err_result(&format!("{}", e))
}
...
/// 创建 MemoryCenter 实例
///
/// 创建一个绑定到指定存储路径和会话的 MemoryCenter 句柄。
...
/// - `project_id` 可为 NULL
#[no_mangle]
pub unsafe extern "C" fn memory_center_new(
    root_path: *const c_char,
    session_id: *const c_char,
    project_id: *const c_char,
) -> *mut MemoryCenterHandle {
    ...
    Box::into_raw(Box::new(MemoryCenterHandle {
        storage,
        runtime,
...
///
/// # Safety
/// `handle` 必须是之前通过 [`memory_center_new`] 创建的有效指针，或 NULL
#[no_mangle]
pub unsafe extern "C" fn memory_center_free(handle: *mut MemoryCenterHandle) {
    if !handle.is_null() {
...
///
/// # Safety
/// `result` 必须是之前由其他 MemoryCenter 操作返回的有效指针，或 NULL
#[no_mangle]
pub unsafe extern "C" fn memory_center_is_ok(result: *const MemoryCenterResult) -> bool {
    if result.is_null() {
        return false;
    }
    (*result).is_ok
...
/// 获取结果中的数据字符串（调用方需用 [`memory_center_free_string`] 释放）
///
/// 数据内容因操作而异：
...
/// `result` 必须是之前由其他 MemoryCenter 操作返回的有效指针，或 NULL
#[no_mangle]
pub unsafe extern "C" fn memory_center_get_data(result: *const MemoryCenterResult) -> *mut c_char {
    if result.is_null() {
        return std::ptr::null_mut();
    }
    (*result)
...
/// 获取结果中的错误消息（调用方需用 [`memory_center_free_string`] 释放）
///
/// # Safety
/// `result` 必须是之前由其他 MemoryCenter 操作返回的有效指针，或 NULL
#[no_mangle]
pub unsafe extern "C" fn memory_center_get_error(result: *const MemoryCenterResult) -> *mut c_char {
    if result.is_null() {
        return std::ptr::null_mut();
    }
    (*result)
...
/// 释放后不得再次使用该指针。
#[no_mangle]
pub unsafe extern "C" fn memory_center_result_free(result: *mut MemoryCenterResult) {
    if !result.is_null() {
        drop(Box::from_raw(result));
    }
}

/// 释放字符串（由 [`memory_center_get_data`] 或 [`memory_center_get_error`] 返回）
///
/// # Safety
/// `s` 必须是由 [`memory_center_get_data`] 或 [`memory_center_get_error`] 返回的字符串指针，或 NULL。
/// 释放后不得再次使用该指针。
#[no_mangle]
pub unsafe extern "C" fn memory_center_free_string(s: *mut c_char) {
    if !s.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn memory_center_retrieve(
    handle: *mut MemoryCenterHandle,
...
/// `handle` 必须是有效的非 NULL 句柄
#[no_mangle]
pub unsafe extern "C" fn memory_center_get_summaries(
    handle: *mut MemoryCenterHandle,
) -> *mut MemoryCenterResult {
    let handle = match handle.as_ref() {
        Some(h) => h,
...
/// `handle` 必须是有效的非 NULL 句柄
#[no_mangle]
pub unsafe extern "C" fn memory_center_render_prompt(
    handle: *mut MemoryCenterHandle,
) -> *mut MemoryCenterResult {
    let handle = match handle.as_ref() {
        Some(h) => h,
...
/// `handle` 必须是有效的非 NULL 句柄
#[no_mangle]
pub unsafe extern "C" fn memory_center_run_compaction(
    handle: *mut MemoryCenterHandle,
    period: u32,
) -> *mut MemoryCenterResult {
    let handle = match handle.as_ref() {
