...
//!
//! 调用方式：直接调 MemoryCenterMcp::pre_compress_hook 方法（非走 MCP 协议层）。
//! 方法返回 Result<String, McpError>，String 为 PreCompressResult 序列化后的 JSON。

use memory_center_mcp::{MemoryCenterMcp, PreCompressParams};
...
async fn test_pre_compress_hook_id_consistency() {
    use memory_center_core::model::ArchivePeriod;
