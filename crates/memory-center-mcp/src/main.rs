...
//! ## 环境变量
//!
//! - `MEMORY_CENTER_ROOT`：存储根目录（默认 `./data`）
...
//! ## v2.36 重构说明
//!
//! 启动期 `build_*` 函数已抽离到 `memory_center_mcp::bootstrap` 模块，
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    // v2.31：CLI 子命令支持
    // 用法：
    //   memory-center-mcp                           → 启动 MCP server（默认）
    //   memory-center-mcp install-rules --client <type> --project-root <path> [--force]
    //   memory-center-mcp --help
    ...
    if args.len() >= 2 {
        match args[1].as_str() {
            ...
            "--help" | "-h" => {
                eprintln!("MemoryCenter MCP server v2.36\n");
                eprintln!("用法:");
                eprintln!("  memory-center-mcp                           启动 MCP server (stdio 传输)");
                eprintln!("  memory-center-mcp install-rules --client <type> --project-root <path> [--force]");
                ...
                eprintln!("\n环境变量:");
                eprintln!("  MEMORY_CENTER_ROOT          存储根目录 (默认 ./data)");
    ...
    tracing_subscriber::fmt()
        ...
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "memory_center_mcp=info".into()),
        )
        .init();
    ...
    let runtime_status = memory_center_mcp::RuntimeStatus {
...
fn run_install_rules_cli(args: &[String]) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        match args[i].as_str() {
            "--client" => {
                ...
                if i >= args.len() {
                    ...
                    eprintln!("用法: memory-center-mcp install-rules --client <catpaw|trae|claude-code> --project-root <path> [--force]");
                    std::process::exit(1);
                }
                client = args[i].clone();
            }
            ...
            "--help" | "-h" => {
                eprintln!("安装 MemoryCenter Rules 模板到 Agent 客户端\n");
                eprintln!("用法: memory-center-mcp install-rules --client <type> --project-root <path> [--force]\n");
                eprintln!("参数:");
                eprintln!("  --client <type>          客户端类型: catpaw / trae / claude-code");
                eprintln!("  --project-root <path>    项目根目录的绝对路径");
                eprintln!("  --force                   强制覆盖已存在的文件（默认 false）\n");
                eprintln!("示例:");
                eprintln!("  memory-center-mcp install-rules --client catpaw --project-root D:/myapp");
                eprintln!("  memory-center-mcp install-rules --client trae --project-root /home/user/myapp");
                eprintln!("  memory-center-mcp install-rules --client claude-code --project-root ./myapp --force");
                return Ok(());
            }
            other => {
    ...
    // 调用公共函数
    match memory_center_mcp::install_rules_to_project(&client, &project_root, force) {
        Ok(result_json) => {
            println!("{result_json}");
            Ok(())
        }
