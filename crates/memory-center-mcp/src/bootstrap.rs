...
//!
//! - `memory-center-mcp` bin（stdio 传输）
//! - `memory-center-server` bin（HTTP 传输，挂载到 axum Router）
//!
//! ## 设计原则
//!
...
use memory_center_core::conflict::{ConflictDetector, HybridDetector};
...
use memory_center_llm::{HttpLlmDetector, LlmDetectorConfig};
...
///
/// - 配置了 `MEMORY_CENTER_EMBEDDER_API_URL` + `API_KEY`：
...
pub fn build_session_search(
    ...
    Option<Arc<memory_center_search::SessionSearchRouter>>,
    &'static str,
    Option<usize>,
) {
...
///    - 按 family 推导 scenario（MEMORY_CENTER_PRESET_SCENARIO 优先，否则 coding/daily）
...
///   （由 `MemoryCenterMcp::get_info()` 注入，客户端把它注入 LLM system prompt）
...
pub fn build_combined_profile() -> Option<CombinedProfile> {
    ...
    // 推导 scenario（环境变量 MEMORY_CENTER_PRESET_SCENARIO 优先）
...
/// 从环境变量构造 Token 估算器（v2.52 阶段 4 新增，v2.54 P16 下沉到 archive-core）
///
/// **v2.54 P16 变更**：函数实现已下沉到 `memory-center-archive-core::build_token_estimator_from_env`，
/// 此处 re-export 保持调用方（MCP/Server 的 3 处调用点）import 路径不变。
///
/// 下沉原因：`build_engine_from_env`（archive-core 的便捷函数）需要内部注入 estimator，
/// sidecar 通过 `build_engine_from_env` 构造引擎，此前因 archive-core 无此函数导致 sidecar 漏注入。
pub use memory_center_archive_core::build_token_estimator_from_env;
