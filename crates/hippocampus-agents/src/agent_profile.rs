...
//!
//! AgentProfile 描述当前使用 Hippocampus 的 Agent 代理工具特性：
//! - family + variant（family / variant 分离）
//! - capabilities（工具调用 / 原生压缩）
//! - 归档策略（是否归档到 Hippocampus / session 前缀）
...
impl AgentProfile {
    ...
    pub fn with_archive_disabled(mut self) -> Self {
        self.archive_to_hippocampus = false;
        self
    }
    ...
    ///
    /// - session_prefix 不能为空
    /// - archive_to_hippocampus=false 时无需校验其他字段（Agent 自管记忆）
